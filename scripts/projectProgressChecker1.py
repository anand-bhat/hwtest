...
def main(argv):
    ...
    parser.add_argument('-l', '--length', type=float, default=0.001, help='Trajectory length (in ns) per WU (default: 0.001)')
    ...
    for run_entry in runs:
        ...
        for clone_entry in clones:
            ...
            if clone_entry.get('terminated', False):
                print('   Not checking progress for terminated Gen')
                continue
            ...
            # If no previous history, check last gen
            if gen == -1:
            ...
            if searchType == 'alternate':
                ...
                latest = alternate_search_wu_check(project, run, clone, gen, gen + 2, maxGensPerClone - 1)                
...
def alternate_search_wu_check(project, run, clone, gen, lower, upper):
    """Alternate search for last completed WU."""
    while lower <= upper:
        response = wu_check(project, run, clone, lower + 2)
        if response[0] == 2:
            return (2, lower + 2, response[1])
