...
def main(argv):
    ...
    # Fetch FAH WU database
    global conn
    conn = fetch_database(project)
        ...
        for run_entry in runs:
            ...
            for clone_entry in clones:
                ...
                if checkFinalGenFirst:
                    ...
                    response = wu_check_sql(project, run, clone, maxGensPerClone - 1)
                    if response[0] == 1:
                ...
                response = wu_check_sql(project, run, clone, gen + 1)
                lastGenDate = '-'
                if response[0] == 0:
                    print('   No progress detected')
                    # Super special case: Check next gen
                    response = wu_check_sql(project, run, clone, gen + 2)
                    if response[0] == 0:
                        print('   No progress detected for sure')
                        # Super super special case: Check next gen
                        response = wu_check_sql(project, run, clone, gen + 3)
                        if response[0] == 0:
...
def fetch_database(project):
    """Create or obtain connection to FAH WU database."""
    conn = sqlite3.connect('FAH-' + str(project) + '.db')

    # Check if WU table already exists
    cur = conn.cursor()
    cur.execute("SELECT name FROM sqlite_master WHERE type='table' AND name='wu'")
...
def wu_check_sql(project, run, clone, gen):
    """Check WU status."""
    # 0 - No results found
    # 1 - Success; not reissued after Ok record
    # 2 - Permantently failed; never completed
    ...
    cur = conn.cursor()
    cur.execute('SELECT log_time, code FROM wu WHERE project=? AND run=? AND clone=? AND gen=? order by log_time desc', (project, run, clone, gen))
    ...
    for result in results:
        print(result)
        if genDate is None:
            genDate = result[0] + ' UTC'
        code = result[1]
        if code in ['Ok', 'Relayed']:
            if faulty or dumped:
                success = True
            #else:
            return (1, result[0] + ' UTC')
        elif code in ['Faulty', 'Faulty 2', 'Failed']:
            faulty = True
            faultCount = faultCount + 1
        #elif code == 'Dumped':
...
def binary_search_wu_check(project, run, clone, lower, upper):
    ...
    while lower <= upper:
        mid = (lower + upper) // 2
        response = wu_check_sql(project, run, clone, mid)
        if response[0] >= 2:
...
def alternate_search_wu_check(project, run, clone, lower, upper, lastGenDate):
    ...
    while lower <= upper:
        response = wu_check_sql(project, run, clone, lower + 2)
        if response[0] >= 2:
            return (response[0], lower + 2, response[1])

        if response[0] == 0:
            # Not found, check previous Gen
            response = wu_check_sql(project, run, clone, lower + 1)
            if response[0] >= 1:
